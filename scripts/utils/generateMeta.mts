...
const releases = await fetch("https://api.github.com/repos/slipcords/Slipper/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "org.slipcords.slipper.metainfo.xml");
...
await fs.writeFile("./dist/org.slipcords.slipper.metainfo.xml", output, "utf-8");
console.log("Updated meta information written to ./dist/org.slipcords.slipper.metainfo.xml");
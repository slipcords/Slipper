    ...
    if (isLinux && nativeSNI) {
        try {
            ...
            if (success) {
                ...
                const pixmap = await getCachedTrayPixmap(trayVariant);
                nativeSNI.setStatusNotifierIcon(pixmap);
                nativeSNI.setStatusNotifierTitle("Slipper");
                ...
                const menuItems = [
                    ...
                    { id: 3, label: "Repair Slipcord", enabled: true, visible: true },
                    { id: 4, label: "Reset Slipper", enabled: true, visible: true },
                    ...
                    switch (id) {
                        ...
                        case 3: // repair slipcord
                            downloadVencordAsar()
                                ...
                                .catch(err => {
                                    console.error("[Tray] Repair Slipcord failed:", err);
                                    dialog.showErrorBox(
                                        "Repair Slipcord failed",
                                        `Could not download Slipcord:\n\n${err instanceof Error ? err.message : String(err)}`
                                    );
                                });
                            break;
                        case 4: // reset Slipper
                            clearData(win);
                            break;
